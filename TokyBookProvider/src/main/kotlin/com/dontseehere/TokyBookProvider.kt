...
class TokyBookProvider : MainAPI() { // all providers must be an instance of MainAPI
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = document.select("div.inside-article").mapNotNull {
            it.toSearchResult()
        }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return document.select("div.inside-article").mapNotNull {
            it.toSearchResult()
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val poster = document.selectFirst("img[loading]")?.attr("src") ?: "https://ww3.pelisplus.to/images/logo2.png"
        ...
        return if (tvType == TvType.TvSeries) {
            ...
            //Log.d("TAG"," jason string is $jasonpre")

            //val jason = parseJson<Jason>(jsonstring)

            //Log.d("TAG","${jasonpre?.last()}")

            //I HAVE TO FIX THIS JSON, WOW JUST WOW
            //val place = 380
            //jasonpre = jasonpre?.substring(0, place) + """ "url": "NA" """ + jasonpre?.substring(place, jasonpre?.length!! );

            jasonpre = jasonpre?.replace("""{
                          "track": 1,
                          "name": "welcome",
            ...
            //change to
            //https://files01.tokybook.com/audio/creativity-inc/1628010881866/001 - Creativity, Inc..mp3
